        ...
        for dom in domains:
            for dreg in dregs:
                for gsize in graph_sizes:
                    for rnd_seed in range(random_seeds):
                        curr_df = pd.read_csv(f'{FOLDER}/{exp}/{dom}/{dreg}/{gsize}/{rnd_seed}.csv')
...
def create_plot(experiment, values_dict, domain, dreg, dict_type='logits', n_bins=11):
    ...
    if dict_type == 'logits':
        ...
        min_val = min(min(values_dict[key]) for key in keys)
        max_val = max(max(values_dict[key]) for key in keys)
    else:
        raise ValueError('Unknown dict type')

    histograms = [np.histogram(values_dict[key], bins=n_bins, range=(min_val, max_val))[0] for key in sorted(keys)]  # range(0, max_epoch+1, PLOT_EVERY_NTH)]
    histograms = np.array(histograms).T[::-1]  # Shape: (n_epochs, n_bins)
    histograms = histograms / histograms.sum(axis=0, keepdims=True)

    plt.figure(figsize=(10, 5))
    my_cmap = copy.copy(plt.get_cmap(COLOR_SCHEME))  # copy the default cmap
    my_cmap.set_bad(my_cmap.colors[0])  # 'grey'
    plot = sns.heatmap(histograms, norm=LogNorm(), cmap=my_cmap,  # if dict_type == 'logits' else None
                       xticklabels=sorted(keys), yticklabels=np.linspace(min_val, max_val, n_bins)[::-1].round(2))  # range(0, max_epoch+1, PLOT_EVERY_NTH)
    if max_epoch > 30000:
        divisor = 5
...
best_idxs = result_df[result_df['violation'] == 0].groupby(['experiment', 'domain', 'd_regular', 'graph_id'])['pred_size'].idxmax()
best_seeds_df = result_df.loc[best_idxs]
best_seeds_df.to_csv('best_seeds.csv')
        ...
        for dreg in dregs:
            ...
            for graph_id in range(nr_of_graphs):
                ...
                for val_name in ['logits', 'preacts']:
                    curr_dir = f'{FOLDER}/{exp}/{dom}/{dreg}/100/{graph_id}/{rnd_seed}/{val_name}'
            ...
            create_plot(exp, logits_dict, dom, dreg, dict_type='logits')
            create_plot(exp, preacts_dict, dom, dreg, dict_type='preacts')
